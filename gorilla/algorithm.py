...
def calc_val(values: dict, first_seq: str, second_seq: str) -> (int, str, str):
    ...
    if len(second_seq) > 0:
        ...
        prev_res = calc_val(values, first_seq, second_seq[:-1])
        first_gap_value = (
    ...
    if len(first_seq) > 0:
        ...
        prev_res = calc_val(values, first_seq[:-1], second_seq)
        second_gap_value = (
    ...
    if len(first_seq) > 0 and len(second_seq) > 0:
        first_last = last(first_seq)
        second_last = last(second_seq)
        prev_res = calc_val(values, first_seq[:-1], second_seq[:-1])
